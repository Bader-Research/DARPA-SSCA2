...
on_one_thread {
   ...
   global_radix = (struct global_memory *) G_MALLOC(sizeof(struct global_memory));
   ...
   global_radix->ranktime = (double *) G_MALLOC(number_of_processors*sizeof(double));
   global_radix->sorttime = (double *) G_MALLOC(number_of_processors*sizeof(double));
   global_radix->totaltime = (double *) G_MALLOC(number_of_processors*sizeof(double));
   size = number_of_processors*(radix*sizeof(int)+sizeof(int *));
   rank_me = (int **) G_MALLOC(size);
   if ((global_radix == NULL) || (key[0] == NULL) || (key[1] == NULL) ||
   ...
   for (i=0;i<number_of_processors;i++) {
     gp_radix[i].rank_ff = (int *) G_MALLOC(radix*sizeof(int)+PAGE_SIZE);
   }
   LOCKINIT(global_radix->lock_Index)
   LOCKINIT(global_radix->rank_lock)
   ALOCKINIT(global_radix->section_lock,MAX_PROCESSORS)
   BARINIT(global_radix->barrier_rank, number_of_processors)
   BARINIT(global_radix->barrier_key, number_of_processors)
   
   for (i=0; i<2*number_of_processors; i++) {
     PAUSEINIT(global_radix->prefix_tree[i].done);
   }

   global_radix->Index = 0;
...
/* POSSIBLE ENHANCEMENT:  Here is where one might distribute the key,
   rank_me, rank, and gp_radix data structures across physically 
   ...
   for (i=0;i<number_of_processors;i++) {
     Place all addresses x such that:
       ...
       &(gp_radix[i]) <= x < &(gp_radix[i+1]) on node i
       &(gp_radix[i].rank_ff[0]) <= x < &(gp_radix[i].rank_ff[radix]) on node i
   }
   start_p = 0;
   i = 0;
   ...
   for (toffset = 0; toffset < number_of_processors; toffset ++) {
     ...
     while ((offset & 0x1) != 0) {
       ...
       index = base + offset;
       Place all addresses x such that:
         &(global_radix->prefix_tree[index]) <= x < 
              &(global_radix->prefix_tree[index + 1]) on node toffset
       base += level;
       level >>= 1;
     }  
   }  */
   ...
   CREATEINIT(number_of_processors)
}   
...
on_one_thread {
   ...
   printf("    0     %16.3f      %16.3f      %16.3f\n",
           global_radix->totaltime[0],global_radix->ranktime[0],
           global_radix->sorttime[0]);
   if (dostats) {
     maxt = avgt = mint = global_radix->totaltime[0];
     maxrank = avgrank = minrank = global_radix->ranktime[0];
     maxsort = avgsort = minsort = global_radix->sorttime[0];
     for (i=1; i<number_of_processors; i++) {
       if (global_radix->totaltime[i] > maxt) {
         maxt = global_radix->totaltime[i];
       }
       if (global_radix->totaltime[i] < mint) {
         mint = global_radix->totaltime[i];
       }
       if (global_radix->ranktime[i] > maxrank) {
         maxrank = global_radix->ranktime[i];
       }
       if (global_radix->ranktime[i] < minrank) {
         minrank = global_radix->ranktime[i];
       }
       if (global_radix->sorttime[i] > maxsort) {
         maxsort = global_radix->sorttime[i];
       }
       if (global_radix->sorttime[i] < minsort) {
         minsort = global_radix->sorttime[i];
       }
       avgt += global_radix->totaltime[i];
       avgrank += global_radix->ranktime[i];
       avgsort += global_radix->sorttime[i];
     }
     ...
     for (i=1; i<number_of_processors; i++) {
       printf("  %3d     %16.3f      %16.3f      %16.3f\n",
               i,global_radix->totaltime[i],global_radix->ranktime[i],
               global_radix->sorttime[i]);
     }
     printf("  Avg     %16.3f      %16.3f      %16.3f\n",avgt,avgrank,avgsort);
     printf("  Min     %16.3f      %16.3f      %16.3f\n",mint,minrank,minsort);
     printf("  Max     %16.3f      %16.3f      %16.3f\n",maxt,maxrank,maxsort);
     printf("\n");
   }

   printf("\n");
   global_radix->starttime = start;
   printf("                 TIMING INFORMATION\n");
#if TIMEA
   printf("Start time                        : %16d\n",
           global_radix->starttime);
   printf("Initialization finish time        : %16d\n",
           global_radix->rs);
   printf("Overall finish time               : %16d\n",
           global_radix->rf);
   printf("Total time with initialization    : %16d\n",
           global_radix->rf-global_radix->starttime);
   printf("Total time without initialization : %16d\n",
           global_radix->rf-global_radix->rs);
#else
   printf("Start time                        : %16.3f\n",
           global_radix->starttime);
   printf("Initialization finish time        : %16.3f\n",
           global_radix->rs);
   printf("Overall finish time               : %16.3f\n",
           global_radix->rf);
   printf("Total time with initialization    : %16.3f\n",
           global_radix->rf-global_radix->starttime);
   printf("Total time without initialization : %16.3f\n",
           global_radix->rf-global_radix->rs);
#endif
   printf("\n");
   ...
   if (test_result) {
     test_sort(global_radix->final);  
   }

#if 1
   for (i=0;i<number_of_processors;i++) {
     free(gp_radix[i].rank_ff);
   }
   free(global_radix->ranktime);
   free(global_radix->sorttime);
   free(global_radix->totaltime);
   free(global_radix);
   free(key[0]);
   free(key[1]);
   free(key_partition);
...
void *slave_sort(void *parg)
{
   ...
   LOCK(global_radix->lock_Index)
     MyNum = global_radix->Index;
     global_radix->Index++;
   UNLOCK(global_radix->lock_Index)
...
/* POSSIBLE ENHANCEMENT:  Here is where one might pin processes to
   ...
   BARRIER(global_radix->barrier_key, number_of_processors) 
...
/* Do 1 iteration per digit.  */
   ...
   rank_ff_mynum = gp_radix[MyNum].rank_ff;
...
/* generate histograms based on one digit */
     ...
     BARRIER(global_radix->barrier_rank, number_of_processors)  

     n = &(global_radix->prefix_tree[MyNum]);
     for (i = 0; i < radix; i++) {
     ...
     base = number_of_processors;
     if ((MyNum & 0x1) == 0) {
        SETPAUSE(global_radix->prefix_tree[base + (offset >> 1)].done);
     }
     while ((offset & 0x1) != 0) {
       offset >>= 1;
       r = n;
       l = n - 1;
       index = base + offset;
       n = &(global_radix->prefix_tree[index]);
       WAITPAUSE(n->done);
       CLEARPAUSE(n->done);
       if (offset != (level - 1)) {
         for (i = 0; i < radix; i++) {
     ...
     if (MyNum != (number_of_processors - 1)) {
       ...
       my_node = &(global_radix->prefix_tree[base + offset]);
       offset >>= 1;
       base += level;
       level >>= 1;
       while ((offset & 0x1) != 0) {
       ...
       their_node = &(global_radix->prefix_tree[base + offset]);
       ...
       my_node = &(global_radix->prefix_tree[(2 * number_of_processors) - 2]);
     }
     offset = MyNum;
     level = number_of_processors;
     base = 0;
     ...
     while (offset != 0) {
       if ((offset & 0x1) != 0) {
         ...
         l = &(global_radix->prefix_tree[base + offset - 1]);
         for (i = 0; i < radix; i++) {
   ...
   if ((MyNum == 0) || (stats)) {
     CLOCK(time6)
     global_radix->ranktime[MyNum] = ranktime;
     global_radix->sorttime[MyNum] = sorttime;
     global_radix->totaltime[MyNum] = time6-time1;
   }
   if (MyNum == 0) {
     global_radix->rs = time1;
     global_radix->rf = time6;
     global_radix->final = to;
   }
#if 1
   free(key_density);
#endif
...

{
   ...
   key_final = (int *) key[global_radix->final];
   printf("\n");
