...
void ocean_main(THREADED)
{
...
#if 0
   while ((ch = getopt(THARGC, THARGV, "n:p:e:r:t:soh")) != -1) {
     switch(ch) {
   ...
   gp_ocean = (struct Global_Private *) G_MALLOC((nprocs+1)*sizeof(struct Global_Private));
   for (i=0;i<nprocs;i++) {
     gp_ocean[i].multi_time = 0;
     gp_ocean[i].total_time = 0;
   }

   global_ocean = (struct global_struct *) G_MALLOC(sizeof(struct global_struct));
...
/* each direction                                                  */
   ...
   for (i=0;i<numlev;i++) {
     ...
     for (j=0;j<xprocs;j++) {
       if (xextra == 0) {
         for (k=0;k<yprocs;k++) {
           gp_ocean[k*xprocs+j].rel_start_x[i] = j * xportion + 1;
           gp_ocean[k*xprocs+j].rel_num_x[i] = xportion;
         }
       } else {
         if (j + 1 > xextra) {
           for (k=0;k<yprocs;k++) {
             lower = xextra * (xportion + 1);
             gp_ocean[k*xprocs+j].rel_start_x[i] = lower + (j - xextra) * xportion + 1;
             gp_ocean[k*xprocs+j].rel_num_x[i] = xportion;
           }
         } else {
           for (k=0;k<yprocs;k++) {
             gp_ocean[k*xprocs+j].rel_start_x[i] = j * (xportion + 1) + 1;
             gp_ocean[k*xprocs+j].rel_num_x[i] = xportion + 1;
           }
         }
       }
     }
     ...
     for (j=0;j<yprocs;j++) {
       if (yextra == 0) {
         for (k=0;k<xprocs;k++) {
           gp_ocean[j*xprocs+k].rel_start_y[i] = j * yportion + 1;
           gp_ocean[j*xprocs+k].rel_num_y[i] = yportion;
         }
       } else {
         if (j + 1 > yextra) {
           for (k=0;k<xprocs;k++) {
             lower = yextra * (yportion + 1);
             gp_ocean[j*xprocs+k].rel_start_y[i] = lower + (j - yextra) * yportion + 1;
             gp_ocean[j*xprocs+k].rel_num_y[i] = yportion;
           }
         } else {
           for (k=0;k<xprocs;k++) {
             gp_ocean[j*xprocs+k].rel_start_y[i] = j * (yportion + 1) + 1;
             gp_ocean[j*xprocs+k].rel_num_y[i] = yportion + 1;
           }
         }
       }
     }
   ...
   for (my_num=0;my_num<nprocs;my_num++) {
     for (i=0;i<numlev;i++) {
       gp_ocean[my_num].rlist[i] = gp_ocean[my_num].rel_start_y[i];
       gp_ocean[my_num].rljst[i] = gp_ocean[my_num].rel_start_x[i];
       gp_ocean[my_num].rlien[i] = gp_ocean[my_num].rlist[i] + gp_ocean[my_num].rel_num_y[i] - 1;
       gp_ocean[my_num].rljen[i] = gp_ocean[my_num].rljst[i] + gp_ocean[my_num].rel_num_x[i] - 1;
       gp_ocean[my_num].iist[i] = gp_ocean[my_num].rel_start_y[i];
       gp_ocean[my_num].ijst[i] = gp_ocean[my_num].rel_start_x[i];
       gp_ocean[my_num].iien[i] = gp_ocean[my_num].iist[i] + gp_ocean[my_num].rel_num_y[i] - 1;
       gp_ocean[my_num].ijen[i] = gp_ocean[my_num].ijst[i] + gp_ocean[my_num].rel_num_x[i] - 1;
       gp_ocean[my_num].pist[i] = gp_ocean[my_num].rel_start_y[i];
       gp_ocean[my_num].pjst[i] = gp_ocean[my_num].rel_start_x[i];
       gp_ocean[my_num].pien[i] = gp_ocean[my_num].pist[i] + gp_ocean[my_num].rel_num_y[i] - 1;
       gp_ocean[my_num].pjen[i] = gp_ocean[my_num].pjst[i] + gp_ocean[my_num].rel_num_x[i] - 1;
  
       if (gp_ocean[my_num].pist[i] == 1) {
         gp_ocean[my_num].pist[i] = 0;
       }
       if (gp_ocean[my_num].pjst[i] == 1) {
         gp_ocean[my_num].pjst[i] = 0;
       }
       if (gp_ocean[my_num].pien[i] == imx[i] - 2) {
         gp_ocean[my_num].pien[i] = imx[i]-1;
       }
       if (gp_ocean[my_num].pjen[i] == jmx[i] - 2) {
         gp_ocean[my_num].pjen[i] = jmx[i]-1;
       }

       if (gp_ocean[my_num].rlist[i] % 2 == 0) {
         gp_ocean[my_num].eist[i] = gp_ocean[my_num].rlist[i];
         gp_ocean[my_num].oist[i] = gp_ocean[my_num].rlist[i] + 1;
       } else {
         gp_ocean[my_num].eist[i] = gp_ocean[my_num].rlist[i] + 1;
         gp_ocean[my_num].oist[i] = gp_ocean[my_num].rlist[i];
       }
       if (gp_ocean[my_num].rljst[i] % 2 == 0) {
         gp_ocean[my_num].ejst[i] = gp_ocean[my_num].rljst[i];
         gp_ocean[my_num].ojst[i] = gp_ocean[my_num].rljst[i] + 1;
       } else {
         gp_ocean[my_num].ejst[i] = gp_ocean[my_num].rljst[i] + 1;
         gp_ocean[my_num].ojst[i] = gp_ocean[my_num].rljst[i];
       }
       if (gp_ocean[my_num].rlien[i] == imx[i]-2) {
         gp_ocean[my_num].rlien[i] = gp_ocean[my_num].rlien[i] - 1;
         if (gp_ocean[my_num].rlien[i] % 2 == 0) {
           gp_ocean[my_num].ojest[i] = gp_ocean[my_num].ojst[i];
           gp_ocean[my_num].ejest[i] = gp_ocean[my_num].ejst[i];
         } else {
           gp_ocean[my_num].ojest[i] = gp_ocean[my_num].ejst[i];
           gp_ocean[my_num].ejest[i] = gp_ocean[my_num].ojst[i];
         }
       }
       if (gp_ocean[my_num].rljen[i] == jmx[i]-2) {
         gp_ocean[my_num].rljen[i] = gp_ocean[my_num].rljen[i] - 1;
         if (gp_ocean[my_num].rljen[i] % 2 == 0) {
           gp_ocean[my_num].oiest[i] = gp_ocean[my_num].oist[i];
           gp_ocean[my_num].eiest[i] = gp_ocean[my_num].eist[i];
         } else {
           gp_ocean[my_num].oiest[i] = gp_ocean[my_num].eist[i];
           gp_ocean[my_num].eiest[i] = gp_ocean[my_num].oist[i];
         }
       }
     }
   }
...
/* initialize constants and variables
   ...
   id is a global_ocean shared variable that has fetch-and-add operations
   performed on it by processes to obtain their pids.   */

   global_ocean->id = 0;
   global_ocean->psibi = 0.0;
   pi = atan(1.0);
   pi = 4.*pi;
...
on_one_thread {
   ...
   printf("    0   %15.0f    %15.0f        %10.3f\n",
          gp_ocean[0].total_time,gp_ocean[0].multi_time,
          gp_ocean[0].multi_time/gp_ocean[0].total_time);

   if (do_stats) {
     min_total = max_total = avg_total = gp_ocean[0].total_time;
     min_multi = max_multi = avg_multi = gp_ocean[0].multi_time;
     min_frac = max_frac = avg_frac = gp_ocean[0].multi_time/gp_ocean[0].total_time;
     for (i=1;i<nprocs;i++) {
       if (gp_ocean[i].total_time > max_total) {
         max_total = gp_ocean[i].total_time;
       }
       if (gp_ocean[i].total_time < min_total) {
         min_total = gp_ocean[i].total_time;
       }
       if (gp_ocean[i].multi_time > max_multi) {
         max_multi = gp_ocean[i].multi_time;
       }
       if (gp_ocean[i].multi_time < min_multi) {
         min_multi = gp_ocean[i].multi_time;
       }
       if (gp_ocean[i].multi_time/gp_ocean[i].total_time > max_frac) {
         max_frac = gp_ocean[i].multi_time/gp_ocean[i].total_time;
       }
       if (gp_ocean[i].multi_time/gp_ocean[i].total_time < min_frac) {
         min_frac = gp_ocean[i].multi_time/gp_ocean[i].total_time;
       }
       avg_total += gp_ocean[i].total_time;
       avg_multi += gp_ocean[i].multi_time;
       avg_frac += gp_ocean[i].multi_time/gp_ocean[i].total_time;
     }
     ...
     for (i=1;i<nprocs;i++) {
       printf("  %3d   %15.0f    %15.0f        %10.3f\n",
              i,gp_ocean[i].total_time,gp_ocean[i].multi_time,
              gp_ocean[i].multi_time/gp_ocean[i].total_time);
     }
