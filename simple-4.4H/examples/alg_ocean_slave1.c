...

{
   ...
   LOCK(locks->idlock)
     procid = global_ocean->id;
     global_ocean->id = global_ocean->id+1;
   UNLOCK(locks->idlock)
...
/* POSSIBLE ENHANCEMENT:  Here is where one might distribute
   ...
   firstcol = gp_ocean[procid].rel_start_x[numlev-1];
   lastcol = firstcol + gp_ocean[procid].rel_num_x[numlev-1] - 1;
   firstrow = gp_ocean[procid].rel_start_y[numlev-1];
   lastrow = firstrow + gp_ocean[procid].rel_num_y[numlev-1] - 1;
   numcols = gp_ocean[procid].rel_num_x[numlev-1];
   numrows = gp_ocean[procid].rel_num_y[numlev-1];
...
BARRIER(bars->sl_prini,nprocs)
   ...
   istart = gp_ocean[procid].rel_start_y[numlev-1];
   iend = istart + gp_ocean[procid].rel_num_y[numlev-1] - 1;
   jstart = gp_ocean[procid].rel_start_x[numlev-1];
   jend = jstart + gp_ocean[procid].rel_num_x[numlev-1] - 1;
   ist = istart;
   ien = iend;
   jst = jstart;
   jen = jend;
...
/* update the shared variable psibi by summing all the psibiprivs
   ...
   global_ocean->psibi = global_ocean->psibi + psibipriv;
 ...
 ***************************************************************/
   ...
   while (!endflag) {
     while ((!dayflag) || (!dhourflag)) {
       ...
       if (nstep == 1) {
         if (procid == MASTER) {
            CLOCK(global_ocean->trackstart)
         }
         if ((procid == MASTER) || (do_stats)) {
           CLOCK(t1);
           gp_ocean[procid].total_time = t1;
           gp_ocean[procid].multi_time = 0;
         }
...
/* update values of psilm array to psilm + psim[2]  */
  ...
  if ((procid == MASTER) || (do_stats)) {
    CLOCK(t1);
    gp_ocean[procid].total_time = t1-gp_ocean[procid].total_time;
  }
}
