...
/*     *******************************************************
   ...
   if ((procid == MASTER) || (do_stats)) {
     CLOCK(multi_end);
     gp_ocean[procid].multi_time += (multi_end - multi_start);
   }

   if (procid == MASTER) {
     global_ocean->psiai=0.0;
   }
...
/* after computing its private sum, every process adds that to the
   ...
   global_ocean->psiai = global_ocean->psiai + psiaipriv;
...
/*      *******************************************************
   ...
   f4 = (-global_ocean->psiai)/(global_ocean->psibi);
